...
def visualizer_rllib(args):
    ...
    # initialize fairness metrics (avg std deviation across lanes)
    num_lanes_3 = env.unwrapped.k.network.num_lanes('3') # lanes in the first bottleneck
    num_lanes_4 = env.unwrapped.k.network.num_lanes('4') # lanes in the second bottleneck
    edge_3_speed_std_devs = [] # will contain the standard deviation of mean lane speeds in edge 3 
    edge_4_speed_std_devs = []
    ...
    for i in range(args.num_rollouts):
        ...
        for _ in range(env_params.horizon):
            ...
            # log the non-empty speeds of the lanes in each lane
            veh_ids_edge_3 = env.unwrapped.k.vehicle.get_ids_by_edge('3') # the first bottleneck
            veh_ids_edge_4 = env.unwrapped.k.vehicle.get_ids_by_edge('4') # the second bottleneck
            ...
            for veh_id in veh_ids_edge_3:
                lane = env.unwrapped.k.vehicle.get_lane(veh_id)
                speed = env.unwrapped.k.vehicle.get_speed(veh_id)
            ...
            # evaluate the mean speeds for each lane in each edge if the list of speeds for that lane is non empty
            mean_speeds_3 = [np.mean(lane_speeds) for lane_speeds in lane_speeds_3_list if lane_speeds]
            # print('mean speeds 3', mean_speeds_3)
            mean_speeds_4 = [np.mean(lane_speeds) for lane_speeds in lane_speeds_4_list if lane_speeds]
            # ignore nans while evaluating std
            edge_3_speed_std_devs.append(np.nanstd(mean_speeds_3))
            # hacky double check that mean speeds 4 isn't empty, not sure why it could even try to take the mean of empty list given conditions above
            if mean_speeds_4:
                edge_4_speed_std_devs.append(np.nanstd(mean_speeds_4))
    ...
    print('Standard deviations of average lane speeds in edge 3')
    print('\nMean and standard deviation across timesteps: {}, {}'.format(np.mean(edge_3_speed_std_devs),
    np.std(edge_3_speed_std_devs)))

    print('Standard deviations of average lane speeds in edge 4')
    print('\nMean and standard deviation across timesteps: {}, {}'.format(np.mean(edge_4_speed_std_devs),
    np.std(edge_4_speed_std_devs)))
    ...
    # Compute lane standard deviations in the lats 500 sec
    print('\nStandard deviations of average lane speeds in edge 3, last 500 seconds')
    print('Mean and standard deviation across timesteps: {}, {}'.format(np.mean(edge_3_speed_std_devs[-500:]),
    np.std(edge_3_speed_std_devs[-500:])))

    print('\nStandard deviations of average lane speeds in edge 4, last 500 seconds')
    print('Mean and standard deviation across timesteps: {}, {}'.format(np.mean(edge_4_speed_std_devs[-500:]),
    np.std(edge_4_speed_std_devs[-500:])))
